...
const COULEUR_POINT = "#B0252E"; //Couleur du point cluster
const LABEL_LEGEND = "Equipement culturel";
const REQ_CQL = "&CQL_FILTER=code_insee=";
const code_insee = API.commune ?? undefined;

mviewer.customLayers.culture = (function () {
...
const _handle = function (clusters, views) {
  if (clusters.length > 0) {
    ...
    var panel = "";
    clusters.forEach((c) => {
      // ICI ON CREE MANUELLEMENT LES FEATURES POUR MUSTACHE ET LE TEMPLATE POUR V3.5 ET ANTERIEUR
      if (c?.properties) {
        // v<3.5
